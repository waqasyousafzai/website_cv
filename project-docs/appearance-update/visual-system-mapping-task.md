...
- **No stale removed/renamed-token references.** Repo-wide grep for `lime` and `Michroma` (outside intentional historical rationale) found:
  ...
  - **8 stale prose mentions of "lime" survive in present-tense doc comments** describing now-gold treatments, one each in `src/components/design/{StatReadout,Card,Radio,Tag,Badge,Checkbox,Input}.tsx` and one in `src/features/cv/data.ts:262`. Unlike `fonts.css`'s comment these are not framed as history — they assert current behavior incorrectly (e.g. "Lime hairline — marks the active/selected card"). All are outside this task's fixable file scope (not `src/styles/tokens/*`, `ds-components.css`, `styles.css`, or `index.html`), so recorded here rather than edited.
...
- **8 present-tense doc comments still say "lime"** in `src/components/design/{StatReadout,Card,Radio,Tag,Badge,Checkbox,Input}.tsx` and `src/features/cv/data.ts:262`, describing treatments that are now gold. New finding (Part A). Harmless (comment-only), outside this task's fixable file scope. Owner: whichever of App Shell Transformation / Route-by-Route Restyling next touches each specific file.
